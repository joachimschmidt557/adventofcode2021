...
pub fn main() !void {
    ...
    const count = try countLanternfish(buffered_reader.reader(), 256);
    std.debug.print("number of lanternfish after 256 days: {}\n", .{count});
}
...
test "example 1" {
    ...
    const after_256_days = try countLanternfish(fbs.reader(), 256);
    try std.testing.expectEqual(@as(u64, 26984457539), after_256_days);
}
