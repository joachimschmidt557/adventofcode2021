...
fn countFurtherPaths(
    ...
    for (neighbors) |neighbor| {
        ...
        const visited_small_cave_twice_before = for (current_path) |x, i| {
            // skip if big cave
            if (std.mem.indexOfScalar(u32, big_caves, x) != null) continue;

            // skip if start
            if (x == 0) continue;

            if (std.mem.indexOfScalarPos(u32, current_path, i + 1, x) != null) break true;
        } else false;
        if (visited_small_cave_twice_before and visited_before and !big_cave) continue;
...
test "example 1" {
    ...
    try std.testing.expectEqual(@as(u64, 36), count);
}
...
test "example 2" {
    ...
    var fbs = std.io.fixedBufferStream(text);
    const count = try countPaths(std.testing.allocator, fbs.reader());
    try std.testing.expectEqual(@as(u64, 103), count);
}
...
test "example 1" {
    ...
    var fbs = std.io.fixedBufferStream(text);
    const count = try countPaths(std.testing.allocator, fbs.reader());
    try std.testing.expectEqual(@as(u64, 3509), count);
}
