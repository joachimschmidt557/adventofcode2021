...
pub fn main() !void {
    ...
    const count = try performPolymerization(allocator, buffered_reader.reader(), 10);
...
fn performPolymerization(gpa: std.mem.Allocator, reader: anytype, steps: u64) !u64 {
    ...
    // pair AB
    var pairs = try allocator.alloc(u32, num_letters * num_letters);
    std.mem.set(u32, pairs, 0);
    ...
    // do steps
    {
        var tmp_pairs = try allocator.alloc(u32, num_letters * num_letters);
        var tmp_last_pair: [2]u8 = undefined;
        var i: u64 = 0;
        while (i < steps) : (i += 1) {
            std.mem.copy(u32, tmp_pairs, pairs);
            ...
            std.mem.copy(u32, pairs, tmp_pairs);
            std.mem.copy(u8, &last_pair, &tmp_last_pair);
        }
    }

    var histogram = [_]u32{0} ** num_letters;
    ...
    const max = std.mem.max(u32, &histogram);
    const min = blk: {
        var min: ?u32 = null;
        for (histogram) |x| {
            if (x > 0) {
...
test "example 1" {
    ...
    const count = try performPolymerization(std.testing.allocator, fbs.reader(), 10);
    try std.testing.expectEqual(@as(u64, 1588), count);
}
